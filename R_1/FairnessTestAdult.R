...
#' 
#' @export
FairnessTestAdult = R6::R6Class("FairnessTestAdult", inherit = MOCClassif,
  ...
  public = list(
   ...
   get_difference = function(){
     ...
     moc_classif = MOCClassif$new(predictor_protected, n_generations = 10L, fixed_features = "income")
     ...
     self$data_tSNE$income = ">50K"
  
     df_merged$diff_from_instance = (df_merged[, ">50K"] - pred_x_interest[, 2])
     df_merged = df_merged[df_merged$`>50K` >= 0.5, ]
     df_merged = df_merged[order(-df_merged$`>50K`),]
   ...
   get_dataset_for_tSNE = function(){
     return(self$data_tSNE)
   }
   
   # get_cfactuals_mean = function(){
   #   # call get difference from here maybe?
   #   data_tSNE = self$data_tSNE
   #   return(mean(data_tSNE$No))
   # }
  )
