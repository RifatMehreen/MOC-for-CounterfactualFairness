...
#'
#' @description 
#' A `Counterfactuals_fairness` object should be created by \link{CounterfactualFairnessMethodClassif}.
...
Counterfactuals_fairness = R6::R6Class("Counterfactuals_fairness",
                              
  public = list(
    
    #' @description 
    #' Creates a new `Counterfactuals_fairness` object.
    ...
    #'  
    #' ???????????????????????????????????????????????????
    #' TODO: change here. There should be three attributes
    #' @param desired (`list(1)` | `list(2)`)\cr
    #'  A `list` with the desired properties of the counterfactuals. For classification tasks it should have two elements 
    #'  `desired_class` and `desired_prob` (\link{CounterfactualFairnessMethodClassif}).
    
    initialize = function(cfactuals, predictor, predictor_protected, x_interest, param_set, desired) {
      ...
      assert_names(names(cfactuals), permutation.of = names(predictor_protected$data$X))
      ...
      # where is the 'make_cfactuals_diff' function located?: utils_counterfactual_fairness
    ...
    #' @description Prints the `Counterfactuals_fairness` object.
    print = function() {
      desired = private$.desired
      cat(nrow(private$.data), "Counterfactual(s) \n \n")
      if (private$predictor$task == "classification") {
        cat("Desired class:", desired$desired_protected_attribute, "\n")
  ...
  private = list(
    ...
    get_pred_column = function() {
      if (private$predictor$task == "classification") {
        private$.desired$desired_protected_attribute
      } else {
        1L
      }
    }
