...
#'   # Train a model
#'   rf = randomForest(Two_yr_Recidivism ~ ., data = compas[-6129L, ])
#'   # Create a predictor object
#'   predictor = iml::Predictor$new(rf, type = "prob")
#'   # Find counterfactuals
#'   moc_classif = MOCCounterfactualFairnessClassif$new(predictor, n_generations = 30L)
#'   cfactuals = moc_classif$find_counterfactuals(
#'     x_interest = compas[6129L, ], protected_attribute_col_name = "ethnicity", desired_protected_attribute = "Caucasian", desired_prob = c(0.5, 1)
#'   )
#'   # Print the results
#'   cfactuals$data
#'   # Plot evolution
...
#' 
#' @export
MOCCounterfactualFairnessClassif = R6::R6Class("MOCCounterfactualFairnessClassif", inherit = CounterfactualFairnessMethodClassif,
   
   public = list(
     #' @description Create a new MOCCounterfactualFairnessClassif object.
     #' @template predictor
     ...
     #'   The number of nearest neighbors to use for the fourth objective. Default is `1L`.
     #' @param weights (`numeric(1) | numeric(k)` | `NULL`)\cr  
     #'   The weights used to compute the weighted average distance for the fourth objective. It is either a single value 
     ...
     #'   
     #'   I have added `NULL` for idx here. idx is the index of the protected attribute
     initialize = function(predictor, epsilon = NULL, fixed_features = NULL, max_changed = NULL, mu = 20L, 
                           ...
                           p_mut_gen = 0.56, p_mut_use_orig = 0.32, k = 1L, idx = NULL, weights = NULL, lower = NULL, upper = NULL, 
     ...
     get_dominated_hv = function() {
       ...
       # TODO: how to change this ref_point?
   ...
   private = list(
     ...
     run = function() {
       ...
       # TODO: check 'ref_point', why is it used
       # min of the probabilities is taken. Then a list named ref_point is made [0.57, 1, 6, 1]
       # private$ref_point = c(min(abs(y_hat_interest - private$desired_prob)), 1, ncol(private$x_interest), 1)
       
       private$.optimizer = moc_fairness_algo(
         predictor = private$predictor,
         x_interest = private$x_interest,
         pred_column = pred_column,
         # change here for the desired protected attribute
         protected_column = private$idx, # which(names(predictor$data$X) == protected_attribute_col_name)
