...
#'   fairness_obj = Fairness$new(predictor, df = compas, column = "ethnicity", row_num = 900L, desired_class = "Caucasian")
#'   # Print the results
...
#' 
#' @export
Fairness_test = R6::R6Class("Fairness_test", inherit = MOCClassif,
  
  public = list(
   #' @description Create a new Fairness_test object.
   #' @template predictor
   ...
   get_difference = function(){
     ...
     rf1 = randomForest(est, data = self$df[-(self$row_num), ])
